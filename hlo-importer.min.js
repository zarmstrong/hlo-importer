let hlodebug=!1;const hloiVer="0.6.1",color1="color: #7bf542",color2="color: #d8eb34",color3="color: #ffffff",color4="color: #cccccc",color5="color: #ff0000";var hlo,userToken;async function doHVExport(hero,act){if(game.modules.get("herovaultfoundry")?.active){let{exportToHVFromPBHLO}=await import("/modules/herovaultfoundry/herovault-min.js");void 0!==exportToHVFromPBHLO&&exportToHVFromPBHLO(hero,act)}}Hooks.on("ready",async function(){"pf2e"!=game.system.id?console.log("%cHLO Importer | %cWrong game system. %cNot enabling.",color1,color5,color4):(console.log("%cHLO Importer | %cinitializing",color1,color4),game.settings.register("hlo-importer","userToken",{name:"User Token (optional)",hint:"Please enter your personal user token. A user token allows external tools (like this one) to access the HLO server and perform export operations.",scope:"world",config:!0,type:String,default:"",onChange:value=>userToken=game.settings.get("hlo-importer","userToken")}),game.settings.register("hlo-importer","debugEnabled",{name:"Enable debug mode",hint:"Debug output will be written to the js console.",scope:"world",config:!0,type:Boolean,default:!1,onChange:value=>hlodebug=game.settings.get("hlo-importer","debugEnabled")})),hlo=new HeroLabImporter(hlodebug),hlodebug=game.settings.get("hlo-importer","debugEnabled"),userToken=game.settings.get("hlo-importer","userToken")}),Hooks.on("renderActorSheet",function(obj,html){if(hlodebug=game.settings.get("hlo-importer","debugEnabled"),"pf2e"!=game.system.id)console.log("%cHLO Importer | %cWrong game system. %cNot adding HLO button to actor sheet.",color1,color5,color4);else{const actor=obj.actor;if(hlodebug&&(console.log("%cHLO Importer | %cPF2e System Version: hlo-importer actor type: "+actor.data.type,color1,color4),console.log("%cHLO Importer | %cCan user modify: "+actor.canUserModify(game.user,"update"),color1,color4)),"character"===actor.data.type&&(0!=actor.canUserModify(game.user,"update")&&!game.modules.get("herovaultfoundry")?.active)){let element=html.find(".window-header .window-title");if(1==element.length){let button=$('<a class="popout" style><i class="fas fa-flask"></i>HLO</a>');userToken=game.settings.get("hlo-importer","userToken"),hlodebug&&console.log("%cHLO Importer | %chlo-importer token: "+userToken,color1,color4),button.on("click",()=>hlo.beginHLOImport(obj.object,userToken)),element.after(button)}}}});class HeroLabImporter{constructor(hlodebug){this.color1="color: #7bf542",this.color2="color: #d8eb34",this.color3="color: #ffffff",this.color4="color: #cccccc",this.color5="color: #ff0000",this.hlodebug=hlodebug,this.heroVaultExport=!1,this.heroVaultPrompt=!1}hloShim(targetActor){this.heroVaultPrompt=!0,userToken=game.settings.get("hlo-importer","userToken");this.beginHLOImport(targetActor,userToken)}beginHLOImport(targetActor,userToken){let applyChanges=!1,hvCheckbox="";this.heroVaultPrompt&&(hvCheckbox='<br><input type="checkbox" id="checkBoxHVExport" name="checkBoxHVExport" ><label for="checkBoxHVExport"> Export this PC to my HeroVau.lt</label><br>'),new Dialog({title:"Herolab Online Import",content:`
        <div>
